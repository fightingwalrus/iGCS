...
/** Updates the file's contents with the contents of the file at `localPath`. If the file is not
 cached then an error will be generated.
 
 @param shouldSteal whether the file at `localPath` should be copied or if it can be moved from its
...
/** Updates the contents of the file to be the bytes store in `data`. If the file is not cached
...
/** Updates the contents of the file with the parameter `string` encoded in UTF8. If the file is not
 cached then an error will be generated.
...
/** The DBFileInfo for the file.
 ...
 Note that the path of a file can change if a conflict occurs, so the value of `file.info.path` is
 not always equal to the path the file was opened at.
 */
